describe('Component: dataList', function () {
    ...
    it('expect template to be replaced', function () {
        expect(directive.find('md-data-table-container').length).toBe(1);
    });
    ...
    it('expect parameter: "config.enableRowSelection" to work', function () {
        ...
        expect(directive.find('table').hasClass("data-table-hide-checkbox")).toBe(true);

        scope.$digest();

        //When enabled should not have the class
        expect(directive.find('table').hasClass("data-table-hide-checkbox")).toBe(false);
    });
    ...
    it('expect parameter: "config.columns" to render the headers in template right', function () {
        var config = {
            columns: [
                ...
                { field: 'value', isNumeric: true, displayName: 'Value', unit: "R$" }
            ]
        };
        ...
        //Check if headers name are right (eq0) is the checkbox
        expect(directive.find('th').eq(1).attr('name')).toBe(config.columns[0].displayName);
        expect(directive.find('th').eq(1).attr('numeric')).toBeDefined();
        expect(directive.find('th').eq(2).attr('name')).toBe(config.columns[1].displayName);
        expect(directive.find('th').eq(2).attr('numeric')).toBeUndefined();
        expect(directive.find('th').eq(3).attr('name')).toBe(config.columns[2].displayName);
        expect(directive.find('th').eq(3).attr('numeric')).toBeDefined();
    ...
    it('expect data to render right ', function () {
        ...
        //Check if data has been rendered right td(0) = checkbox, so we skip it
        expect(directive.find('tbody').find('tr').eq(0).find('td').eq(1).html()).toBe(data[0].id.toString());
        expect(directive.find('tbody').find('tr').eq(0).find('td').eq(2).html()).toBe(data[0].name.toString());
        expect(directive.find('tbody').find('tr').eq(0).find('td').eq(3).html()).toBe(data[0].value.toString());

        expect(directive.find('tbody').find('tr').eq(1).find('td').eq(1).html()).toBe(data[1].id.toString());
        expect(directive.find('tbody').find('tr').eq(1).find('td').eq(2).html()).toBe(data[1].name.toString());
        expect(directive.find('tbody').find('tr').eq(1).find('td').eq(3).html()).toBe(data[1].value.toString());
    ...
    it('expect "orderField" to order data by its value', function () {
        ...
        scope.orderField = "-id";
        scope.$digest();
        expect(directive.find('tbody').find('tr').eq(0).find('td').eq(1).html()).toBe(data[1].id.toString());
        expect(directive.find('tbody').find('tr').eq(1).find('td').eq(1).html()).toBe(data[0].id.toString());
    });
